...
spacex_df = pd.read_csv("C:/Users/User/Desktop/Data Science/IBM Course/Course 10/spacex_launch_dash.csv")
max_payload = spacex_df['Payload Mass (kg)'].max()
min_payload = spacex_df['Payload Mass (kg)'].min()

Landing_Outcome=[]
def assign_landing_outcome(launch_outcome):
    if launch_outcome == 1:
        return 'Success'
    else:
        return 'Failure'
    
spacex_df['Landing Outcome'] = spacex_df['class'].apply(assign_landing_outcome)
...
def get_pie_chart(entered_site):
    if entered_site == "ALL":
        ...
        filtered_df = filtered_df.groupby('Landing Outcome').size().reset_index(name='Class Count')
